...
public class QuestionServiceImpl extends ServiceImpl<QuestionMapper, Question>
    ...
    public void validQuestion(Question question, boolean add) {
        ...
        //创建时，不能为空
        if (add) {
            ThrowUtil.throwIf(StringUtils.isAnyBlank(title, content, tags), ErrorCode.PARAMS_ERROR);
        }
        ...
        if (StringUtils.isNotBlank(judgeConfig) && judgeConfig.length() > 8192) {
            throw new BusinessException(ErrorCode.PARAMS_ERROR, "判题配置过长过长");
        }
    ...
    public QueryWrapper<Question> getQueryWrapper(QuestionQueryRequest questionQueryRequest) {
        ...
        if (StringUtils.isNotEmpty(searchText)) {
            queryWrapper.and(wrapper -> wrapper.like("title", searchText).or().like("content", searchText).or().like("tags", searchText));
        }
    ...
    public Page<QuestionVO> getQuestionVOPage(Page<Question> questionPage, HttpServletRequest request) {
        ...
        // 填充信息
        List<QuestionVO> questionVOList = questionList.stream().filter(question -> !Integer.valueOf(1).equals(question.getIsDelete())).map(question -> {
            QuestionVO questionVO = QuestionVO.objToVo(question);
            Long userId = question.getUserId();
            User user = null;
    ...
    public Page<Question> searchQuestionFromES(QuestionQueryRequest questionQueryRequest) {
        ...
        if (content != null) {
            boolQueryBuilder.filter(QueryBuilders.termQuery("content", content));
        }
        if (answer != null) {
            boolQueryBuilder.filter(QueryBuilders.termQuery("answer", answer));
        }
        if (CollUtil.isNotEmpty(tags)) {
            for (String tag : tags) {
        ...
        NativeSearchQuery nativeSearchQuery = nativeSearchQueryBuilder.withQuery(boolQueryBuilder).withSort(sortBuilder).withPageable(PageRequest.of(current, pageSize)).build();
//        if (boolQueryBuilder.hasClauses()) {
//            nativeSearchQuery = nativeSearchQueryBuilder.withQuery(boolQueryBuilder).withSort(sortBuilder).withPageable(PageRequest.of(current, pageSize)).build();
//        } else {
//            nativeSearchQuery = nativeSearchQueryBuilder.withQuery(QueryBuilders.matchAllQuery()).build();
//        }
        SearchHits<QuestionESDTO> searchHits = elasticsearchRestTemplate.search(nativeSearchQuery, QuestionESDTO.class);

        List<Question> questionList = searchHits.getSearchHits()
                .stream()
                .map(questionESDTOSearchHit -> QuestionESDTO.dtoToObj(questionESDTOSearchHit.getContent()))
                .collect(Collectors.toList());
        ...
        questionPage.setTotal(pageSize);
        return questionPage;
    }
